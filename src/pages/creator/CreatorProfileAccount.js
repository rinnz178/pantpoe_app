...
const CreatorProfileAccountSetting = ({ user, freshData }) => {
  ...
  React.useEffect(() => {
    ...
    if (user?.user_info.dob) {
      let user_dob = new Date(user?.user_info.dob);
    ...
    setState((prev) => ({
      ...prev,
      name: user?.user_info.user.name,
      email: user?.user_info.user.email,
      day: birthday.day,
      month: birthday.month,
      year: birthday.year,
      gender: user?.user_info.gender || user?.user_info.user_info?.gender || "",

      list: user?.user_info.socials
        ? changeSocials(user?.user_info.socials)
        : [],
      region_id: user?.user_info.region?.id || 0,
      address:
        user?.user_info.address === "null" ? " " : user?.user_info.address,
      phone_1: `+${user?.user_info.user.phone_no}`,
      socials: user?.user_info.socials ? user?.user_info.socials : [],
    }));
  }, [user]);
  // console.log(user)
        ...
        <Box className={classes.cusFormControl}>
          ...
          <Box className={classes.cusFormControl}>
            ...
            <TextField
              ...
              value={user?.data?.user_info?.user?.email}
              name="email"
...
export default CreatorProfileAccountSetting;
