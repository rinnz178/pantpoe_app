...
export const checkAuth = () => {
  ...
  if (userObj) {
    let user_arr = JSON.parse(userObj);
    let login_time = user_arr.seen;
    console.log(login_time);
    let now = new Date().getTime();
    let different = now - login_time;
    console.log(different);
    let h = Math.floor(different / 1000 / 60 / 60);
    //return true;
    console.log(h);
    if (h === 24) {
...
// note payement
// success route for payment success
// https://pantpoe.com/payment/success?merchantOrderId=43&state=SUCCESS

// failed route for payment fail
//https://pantpoe.com/payment/fail?merchantOrderId=43&state=FAIL
